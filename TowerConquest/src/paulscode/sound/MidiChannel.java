...
/**
 * The MidiChannel class provides an interface for playing MIDI files, using the JavaSound API. For more information about the JavaSound API, visit http://java.sun.com/products/java-media/sound/
 * <br><br>
...
public class MidiChannel implements MetaEventListener {
    ...
    /**
     * Runs the assigned sequence, passing information on to the synthesizer for playback.
     */
    ...
    /**
     * Constructor: Defines the basic source information. The fourth parameter, 'identifier' should look like a filename, and it must have the correct extension (.mid or .midi).
     *
     * @param toLoop Should playback loop or play only once?
     * @param sourcename Unique identifier for this source.
     ...
     *
     * @param filename Filename or identifier of a MIDI sequence to remove from the queue.
     */
    ...
    /**
     * Changes the basic information about the MIDI source. This method removes any queued filenames/URLs from the list of MIDI sequences that would have played after the current sequence ended.
     *
     * @param toLoop Should playback loop or play only once?
     * @param sourcename Unique identifier for this source.
     * @param filename Name of the MIDI file to play.
    ...
    /**
     * Attempts to load the default sequencer. If it fails, then other common sequencers are tried. If none can be loaded, then variable 'sequencer' remains null.
     */
    ...
    /**
     * Loads the MIDI sequence form the specified URL, and sets the sequence. If variable 'sequencer' is null or an error occurs, then variable 'sequence' remains null.
     *
    ...
    private MidiDevice openMidiDevice(String containsString) {
        ...
        for (MidiDevice.Info midiDevice : midiDevices) {
            ...
            if (device != null && midiDevice.getName().contains(containsString)) {
                message("    Found MIDI device named '" + midiDevice.getName() + "'");
    ...
    private class FadeThread extends SimpleThread {
        ...
        @Override
        /**
         * Runs in the background, timing fade in and fade out, changing the sequence, and issuing the appropriate volume change messages.
         */
        public void run() {
            while (!dying()) {
