...
public class ParticleSwordSlash1 extends Particle {
	...
	public ParticleSwordSlash1(final int k, final int x, final int y, final byte f) {
		super(k, x, y, f);
		this.frame = 0;
	...
	public void update() {
		...
		if (this.frameDuration <= 0) {
			this.frameDuration = 50;
			if (this.frame < PARTICLE_SPRITE[Globals.PARTICLE_SWORD_SLASH1].length - 1) {
				this.frame++;
			}
		}
	}
	...
	public void draw(final Graphics2D g) {
		...
		if (this.frame >= PARTICLE_SPRITE[Globals.PARTICLE_SWORD_SLASH1].length) {
			return;
		}
		final BufferedImage sprite = PARTICLE_SPRITE[Globals.PARTICLE_SWORD_SLASH1][this.frame];
