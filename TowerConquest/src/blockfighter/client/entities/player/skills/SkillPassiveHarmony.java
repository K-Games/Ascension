...
public class SkillPassiveHarmony extends Skill {
    ...
    public void drawInfo(final Graphics2D g, final int x, final int y) {
        final int boxHeight = (this.level < 30) ? 210 : 165, boxWidth = 370;
        int drawX = x, drawY = y;
        ...
        g.drawString("Gain " + (0.1 + (this.level * 0.1)) + "% of Max HP as bonus damage.", drawX + 10, drawY + 155);

        if (this.level < 30) {
            g.drawString("[Level " + (this.level + 1) + "]", drawX + 10, drawY + 180);
            g.drawString("Gain " + (0.1 + ((this.level + 1) * 0.1)) + "% of Max HP as bonus damage.", drawX + 10, drawY + 200);
        }
    }
