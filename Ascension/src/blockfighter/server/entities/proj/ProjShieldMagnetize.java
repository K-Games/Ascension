...
public class ProjShieldMagnetize extends Projectile {
    ...
    public int calculateDamage(final boolean isCrit) {
        ...
        double baseValue = owner.getSkill(Globals.SHIELD_MAGNETIZE).getSkillData().getBaseValue();
        double multValue = owner.getSkill(Globals.SHIELD_MAGNETIZE).getSkillData().getMultValue();
        double baseDef = owner.getSkill(Globals.SHIELD_MAGNETIZE).getCustomValue(0);
        double multDef = owner.getSkill(Globals.SHIELD_MAGNETIZE).getCustomValue(1);
        ...
        damage *= (owner.isSkillMaxed(Globals.SHIELD_MAGNETIZE)) ? owner.getSkill(Globals.SHIELD_MAGNETIZE).getCustomValue(2) : 1;
