...
public class SkillShieldRoar extends Skill {
    ...
    public void updateDesc() {
        this.skillCurLevelDesc = new String[]{
            "Deals " + Globals.NUMBER_FORMAT.format((BASE_VALUE + MULT_VALUE * this.level) * 100) + "% + Defense multiplied by " + Globals.NUMBER_FORMAT.format(CUSTOM_VALUES.get(CUSTOM_DATA_HEADERS[3]) * (CUSTOM_VALUES.get(CUSTOM_DATA_HEADERS[1]) + CUSTOM_VALUES.get(CUSTOM_DATA_HEADERS[2]) * this.level)) + " damage."
        };
        this.skillNextLevelDesc = new String[]{
            "Deals " + Globals.NUMBER_FORMAT.format((BASE_VALUE + MULT_VALUE * (this.level + 1)) * 100) + "% + Defense multiplied by " + Globals.NUMBER_FORMAT.format(CUSTOM_VALUES.get(CUSTOM_DATA_HEADERS[3]) * (CUSTOM_VALUES.get(CUSTOM_DATA_HEADERS[1]) + CUSTOM_VALUES.get(CUSTOM_DATA_HEADERS[2]) * (this.level + 1))) + " damage."
        };
        this.maxBonusDesc = new String[]{
            "Enemies are stunned for " + Globals.NUMBER_FORMAT.format(CUSTOM_VALUES.get(CUSTOM_DATA_HEADERS[0]) / 1000) + " seconds."
        };
    }
