...
public class SkillPassiveBowMastery extends Skill {
    ...
    public void drawInfo(final Graphics2D g, final int x, final int y) {
        ...
        g.drawString("Additional " + (30 + this.level * 4) + "% Critical Hit Damage.", drawX + 10, drawY + 175);
        if (this.level < 30) {
            g.drawString("[Level " + (this.level + 1) + "]", drawX + 10, drawY + 200);
            g.drawString("Additional " + (30 + (this.level + 1) * 4) + "% Critical Hit Damage.", drawX + 10, drawY + 220);
        }
    }
}
