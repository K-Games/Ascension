...
public class SkillSwordCinder extends Skill {
    ...
    public void updateDesc() {
        this.skillCurLevelDesc = new String[]{
            ...
            "Burning enemies take " + Globals.NUMBER_FORMAT.format(CUSTOM_VALUES.get(CUSTOM_DATA_HEADERS[1]) * 100 * this.level) + "% increased damage."
        };
        this.skillNextLevelDesc = new String[]{
            "Deals " + Globals.NUMBER_FORMAT.format((BASE_VALUE + MULT_VALUE * (this.level + 1)) * 100) + "% damage.",
            "Burning enemies take " + Globals.NUMBER_FORMAT.format(CUSTOM_VALUES.get(CUSTOM_DATA_HEADERS[1]) * 100 * (this.level + 1)) + "% increased damage."
        };
        this.maxBonusDesc = new String[]{
            "Burn also deals " + Globals.NUMBER_FORMAT.format(CUSTOM_VALUES.get(CUSTOM_DATA_HEADERS[2]) * 100) + "% damage per second for " + Globals.TIME_NUMBER_FORMAT.format(CUSTOM_VALUES.get(CUSTOM_DATA_HEADERS[0]) / 1000) + " seconds.",
            "Firebrand has " + Globals.NUMBER_FORMAT.format(CUSTOM_VALUES.get(CUSTOM_DATA_HEADERS[3]) * 100) + "% Critical Hit Chance."
        };
    }
