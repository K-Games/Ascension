...
public class SkillSwordVorpal extends Skill {
    ...
    public void updateDesc() {
        this.skillCurLevelDesc = new String[]{
            ...
            "Critical Hits deal additional +" + Globals.NUMBER_FORMAT.format((CUSTOM_VALUES.get(CUSTOM_DATA_HEADERS[1]) + CUSTOM_VALUES.get(CUSTOM_DATA_HEADERS[2]) * this.level) * 100) + "% Critical Damage.",
            HYPER_STANCE_DESC
        };
        this.skillNextLevelDesc = new String[]{
            "Deals " + Globals.NUMBER_FORMAT.format((BASE_VALUE + MULT_VALUE * (this.level + 1)) * 100) + "% damage per hit.",
            "Critical Hits deal additional +" + Globals.NUMBER_FORMAT.format((CUSTOM_VALUES.get(CUSTOM_DATA_HEADERS[1]) + CUSTOM_VALUES.get(CUSTOM_DATA_HEADERS[2]) * (this.level + 1)) * 100) + "% Critical Damage.",
            HYPER_STANCE_DESC
        };
        this.maxBonusDesc = new String[]{
            "This attack has +" + Globals.NUMBER_FORMAT.format(CUSTOM_VALUES.get(CUSTOM_DATA_HEADERS[0]) * 100) + "% Critical Hit Chance.",
            "Strikes 5 times."
        };
    }
